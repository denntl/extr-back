...
const utils = {
    getSearch: (base) => {
        ...
        if (!params.get("externalId") && externalId) {
...
class AppService {
    ...
    redirectToOffer(appEntity) {
        window.location.href = appEntity.redirect + '&onesignal=ok';
    }
}
...
class ButtonEntity {
    get baseEl() {
        return document.getElementsByClassName("install-button")
    }
    get loadingEl() {
        ...
        setTimeout(function () {
            ...
            self._interval = setInterval((() => {
                if (progress.length) {
                    ...
                    self.stopInstalling(buttonEntity, installerEntity);
                }
            }), setting.installing.interval);
        }, 3000)

    }
    stopInstalling(buttonEntity, installerEntity) {
        clearInterval(this._interval);
        installerEntity.installed();
        // buttonEntity.setOpen();
        const event = new CustomEvent('stopInstalling')
        window.dispatchEvent(event)
    }
}
...
window.addEventListener("DOMContentLoaded", (() => {
    ...
    if ("standalone" !== utils.displayMode()) {
            ...
            const check = async () => {
                if ("getInstalledRelatedApps" in window.navigator) {
                    ...
                    if (result.length > 0) {
                        ...
                        installerEntity.installed();
                        installerService.stopInstalling(buttonEntity, installerEntity);
                    }
                }
            };
            await check();
        ...
        appService.showInstallBody(appEntity);
        Array.from(buttonEntity.baseEl).forEach(function(elem) {
            elem.addEventListener("click", (async () => {
                ...
                if ("installed" === installerEntity.status) {
                    ...
                    url.searchParams.set("onsignal", 'ok');
                    window.open(url.toString(), "_blank")
                }
                if ("iOS" === uaParser.getOS().name || true) {
