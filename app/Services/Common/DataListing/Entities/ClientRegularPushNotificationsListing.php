...
class ClientRegularPushNotificationsListing extends ClientSinglePushNotificationsListing
{
    protected function getFieldCollector(): FieldCollector
    {
        return FieldCollector::init()->setBatch([
            ...
            FieldListAsString::init('geo', 'push_notifications.geo', 'Гео')
                ->setListName('geos'),
            FieldString::init('send_time', 'push_notifications.time', 'Время отправки'),
            FieldList::init('delayed', 'push_notifications.is_delayed', 'Отложенный')
                ->setListName('delayed'),
            FieldList::init('status', 'push_notifications.is_active', 'Статус')
    ...
    {
        $commonFilter = [
            new FilterDTO('type', FilterOperator::Equal, Type::Regular->value),
    ...
    {
        return [PermissionName::ClientRegularPushNotificationRead];
    }

    protected function getListItems(): array
    {
        $parent = parent::getListItems();
