...
class CreateInvoiceWithDeclinedPaymentTest extends TestCase
{
    use WithFaker;
    ...
    public function testSuccess()
    {
        ...
        $this->assertEquals(Status::Declined->value, $currentPayment->status);

        $balanceTransaction = BalanceTransaction::query()->where('id', $currentPayment->balance_transaction_id)->firstOrFail();
        $this->assertEquals(Status::Declined->value, $balanceTransaction->status);
        $this->assertNotEquals($customerRealPaidAmount, $balanceTransaction->amount);

        /**
         * Проверяем что баланс не поменялся
         */
        $this->assertEquals($currentCompanyBalance, $companyBalanceService->getBalanceByCompanyId($company->id, Type::Balance));
    }
}
