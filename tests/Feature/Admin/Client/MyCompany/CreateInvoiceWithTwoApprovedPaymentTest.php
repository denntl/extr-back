...
class CreateInvoiceWithTwoApprovedPaymentTest extends TestCase
{
    use WithFaker;
    ...
    public function testSuccess()
    {
        ...
        /** Second payment config **/
        $customerSecondRealPaidAmount = 3;
        $secondPaymentId = $this->faker->numberBetween(1000000000, 9999999999);
        ...
        $requestData = [
            ...
            "order_description" => "Balance transaction #104",
            "order_id" => (string) $currentPayment->balance_transaction_id,
            "outcome_amount" => $customerSecondRealPaidAmount,
            "outcome_currency" => "usdttrc20",
            "parent_payment_id" => null,
            "pay_address" => "TYTzm2BGQ5pANtNvTaWnZdcqoetR9bQH1a",
            "pay_amount" => 100,
        ...
        $currentPayment = Payment::query()->where('payment_id', (string) $secondPaymentId)->firstOrFail();
        $this->assertEquals(Status::Approved->value, $currentPayment->status);

        $balanceTransaction = BalanceTransaction::query()->where('id', $currentPayment->balance_transaction_id)->firstOrFail();
        $this->assertEquals(Status::Approved->value, $balanceTransaction->status);
        $this->assertEquals($customerSecondRealPaidAmount, $balanceTransaction->amount);

        /**
         * Проверка общей суммы по инвойсу
         */
        $approvedTransactions = BalanceTransaction::query()
            ->whereIn('id', Payment::query()->where('invoice_id', $invoiceId)->pluck('balance_transaction_id'))
            ->where('status', Status::Approved->value)
            ->get();
        $totalApprovedAmount = $approvedTransactions->sum('amount');
        $this->assertEquals($customerRealPaidAmount + $customerSecondRealPaidAmount, $totalApprovedAmount);

        /**
         * Проверяем что баланс поменялся
         */
        $this->assertEquals(
            $currentCompanyBalance + $customerRealPaidAmount + $customerSecondRealPaidAmount,
            $companyBalanceService->getBalanceByCompanyId($company->id, Type::Balance)
        );
    }
}
